...
const allowedStatuses: Record<Exclude<AdminSection, "payments">, string[]> = {
...
export async function GET() {
  ...
  try {
    ...
    const [applications, partners, payments, appointments, documents, applicationDocuments] = await Promise.all([
      ...
      db.prepare("SELECT * FROM partner_leads ORDER BY created_at DESC, id DESC").all(),
      db.prepare("SELECT * FROM payments WHERE status = 'paid' AND razorpay_payment_id IS NOT NULL ORDER BY created_at DESC, id DESC").all(),
      db.prepare("SELECT * FROM appointments ORDER BY created_at DESC, id DESC").all(),
...
export async function PATCH(request: Request) {
  ...
  if (!section || section === "payments" || !id || !status || !allowedStatuses[section]?.includes(status)) return Response.json({ error: "Invalid update." }, { status: 400 });
...
export async function DELETE(request: Request) {
  ...
  if (!db) return Response.json({ error: "Database binding DB is not configured." }, { status: 503 });
  await ensureDatabaseSchema(db);
  const body = (await request.json()) as { section?: AdminSection; id?: number; ids?: number[] };
  const section = body.section;
  const ids = [...new Set((body.ids || (body.id ? [body.id] : [])).map(Number))]
    .filter((id) => Number.isInteger(id) && id > 0)
    .slice(0, 100);
  if (!section || !ids.length || !["applications", "partners", "payments", "appointments"].includes(section)) return Response.json({ error: "Invalid delete request." }, { status: 400 });
  try {
    const placeholders = ids.map(() => "?").join(",");
    if (section === "applications") {
      const files = await db.prepare(`SELECT storage_key FROM documents WHERE application_id IN (${placeholders})`).bind(...ids).all<{storage_key:string}>();
      ...
      await db.batch([
        db.prepare(`DELETE FROM documents WHERE application_id IN (${placeholders})`).bind(...ids),
        db.prepare(`DELETE FROM applications WHERE id IN (${placeholders})`).bind(...ids),
      ]);
    } else {
      const table = section === "partners" ? "partner_leads" : section;
