...
export function Header({ active = "" }: { active?: string }) {
  ...
  const links = [["Home","/","fa-house"],["About Us","/about","fa-circle-info"],["Get Help","/get-help","fa-hand-holding-medical"],["Our Impact","/#impact","fa-chart-line"],["Partner With Us","/partner","fa-handshake"],["Resources","/resources","fa-book-open"]];
  return <header><div className="topbar"><div><span><i className="fa-solid fa-phone"/> +91 80000 12345</span><span><i className="fa-solid fa-envelope"/> info@beyonddisability.org</span><span className="topbar-message">Building an inclusive Uttar Pradesh, one family at a time.</span></div></div><div className="nav"><Logo/><label className="mobile-language" data-no-translate><i className="fa-solid fa-globe" aria-hidden="true"/><span aria-hidden="true">{language.toUpperCase()}</span><i className="fa-solid fa-chevron-down" aria-hidden="true"/><select aria-label="Select language" value={language} onChange={e=>setLanguage(e.target.value as Lang)}>{languages.map(item=><option value={item.locale} key={item.locale}>{item.native_name}</option>)}</select></label><button className="menu-toggle" aria-label="Toggle navigation" aria-expanded={menuOpen} onClick={()=>setMenuOpen(!menuOpen)}><i className={`fa-solid ${menuOpen?'fa-xmark':'fa-bars'}`} aria-hidden="true"/></button><nav className={menuOpen?'open':''} aria-label="Primary navigation">{links.map(([label,href])=><Link className={active===label?"active":""} href={href} key={label} onClick={()=>setMenuOpen(false)}><span>{label}</span></Link>)}</nav><div className="nav-actions"><select className="language" aria-label="Select language" value={language} onChange={e=>setLanguage(e.target.value as Lang)}>{languages.map(item=><option value={item.locale} key={item.locale}>{item.native_name}</option>)}</select><Link className="btn btn-gold compact" href="/donate"><i className="fa-solid fa-heart" aria-hidden="true"/>Donate</Link></div></div></header>;
}

export function Button({ href, children, tone="gold", icon }: { href:string; children:ReactNode; tone?:"gold"|"navy"|"outline"|"green"; icon?:string }) {
  return <Link href={href} className={`btn btn-${tone}`}>{icon&&<i className={`fa-solid ${icon}`} aria-hidden="true"/>}<span>{children}</span></Link>;
}
...
export function Footer(){return <footer><div className="footer-banner"><div><p>Need guidance?</p><h2>Our team is here to listen.</h2></div><Button href="/get-help" tone="outline" icon="fa-arrow-right">Get support</Button></div><div className="footer-grid"><div className="footer-brand"><Logo/><p>Removing barriers and creating pathways to independence for children with disabilities and their families.</p><div className="social"><a href="tel:+918000012345" aria-label="Call us"><i className="fa-solid fa-phone"/></a><a href="mailto:info@beyonddisability.org" aria-label="Email us"><i className="fa-solid fa-envelope"/></a><a href="https://wa.me/918000012345" target="_blank" rel="noreferrer" aria-label="WhatsApp us"><i className="fa-brands fa-whatsapp"/></a></div></div><div><h4>Explore</h4><Link href="/about">About our work</Link><Link href="/get-help">Get support</Link><Link href="/#impact">Our impact</Link><Link href="/partner">CSR partnerships</Link></div><div><h4>Programmes</h4><Link href="/about#services">Hearing support</Link><Link href="/about#services">Digital access</Link><Link href="/about#services">Therapy assistance</Link><Link href="/resources">Resources</Link></div><div className="footer-contact"><h4>Contact</h4><a href="tel:+918000012345"><i className="fa-solid fa-phone"/><span>+91 80000 12345<small>Mon–Sat, 9:00–18:00</small></span></a><a href="mailto:info@beyonddisability.org"><i className="fa-solid fa-envelope"/><span>info@beyonddisability.org</span></a><span><i className="fa-solid fa-location-dot"/> Kanpur, Uttar Pradesh</span></div></div><div className="copyright"><span>© 2026 Beyond Disability Foundation</span><span><Link href="/resources#privacy">Privacy</Link> · <Link href="/resources#reports">Transparency</Link></span></div></footer>}
...
export function ApplyForm(){
 ...
 async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setStatus("Submitting…");const form=e.currentTarget;try{const res=await fetch("/api/applications",{method:"POST",body:new FormData(form)});const data=await res.json() as {reference?:string;error?:string};if(!res.ok){setStatus(data.error||"Please check the details and try again.");return}form.reset();setStatus(`Application received successfully. Reference: ${data.reference}`)}catch{setStatus("Network error. Please check your connection and try again.")}}
